...
export class ViewComponent implements OnInit, OnDestroy {
  ...
  renderPDF(inv) {
    ...
    const signres = pdf.autoTableHtmlToJson(signtable, true);
    const totalres = pdf.autoTableHtmlToJson(totaltable, true);
    ...
    for (let i = 0; i < result.length; i++) {
      ...
      pdf.autoTable(totalres.columns, totalres.rows, {
        theme: 'grid',
        styles: {
          fontSize: 9,
          overflow: 'linebreak',
      ...
      pdf.autoTable(signres.columns, signres.rows,
        {
